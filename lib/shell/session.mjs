...
/**
 * PHP adayları — terminalde `$PHP` bu sırayla aranıyor.
 *
 * ⚠ NEDEN GEREKLİ: kısayol düğmeleri `"$PHP" artisan …` yazıyor ama `PHP`
 * değişkeni yalnızca LARAVEL YAYIN betiğinde tanımlıydı. Terminalde boş
 * kaldığı için komut `"" artisan optimize:clear` hâline geliyor ve kabuk
 * "command not found" diyordu — canlıda görüldü.
 *
 * Liste `laravel-core.phpCandidates`'in yedek kısmıyla aynı. Oradaki sürüm
 * tespiti domaine bağlı (bir cPanel çağrısı gerektiriyor); terminal domain
 * bilmediği için burada yalnızca dosya varlığına bakılıyor. Kullanıcı belirli
 * bir sürüm istiyorsa tam yolu kendisi yazabiliyor.
 */
export const PHP_CANDIDATES = [
 ...
 */
export function buildScript(command, { cwd = null, home = '$HOME', php = true } = {}) {
  ...
  return [
    ...
    /*
     * `$PHP` ve `$COMPOSER` kullanılabilir hâle getiriliyor.
     *
     * Kısayollar `"$PHP" artisan …` yazıyor; bu değişken tanımsızsa komut
     * `"" artisan …` olup "command not found" veriyordu. Paylaşımlı hostingde
     * `php` çoğu zaman PATH'te yok ya da yanlış sürümü gösteriyor, o yüzden
     * bilinen konumlar sırayla deneniyor. `export`: kullanıcının komutu bir
     * alt kabuk açarsa orada da görünsün.
     */
    ...(php ? [
      'PHP=""',
      `for p in ${PHP_CANDIDATES.map(shellQuote).join(' ')}; do [ -x "$p" ] && PHP="$p" && break; done`,
      ...
      '[ -n "$COMPOSER" ] || COMPOSER=$(command -v composer 2>/dev/null)',
      'export PHP COMPOSER',
    ] : []),
    /*
