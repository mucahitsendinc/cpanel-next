...
export async function ensureWorker(ctx) {
  ...
  if (String(installed ?? '').trim() !== WORKER_VERSION || !filesPresent) {
    for (const d of [WORKER_DIR, JOBS, RUNNING, RESULTS]) {
 ...
 */
export async function execViaWorker(ctx, body, { onProgress, timeout = 25 * 60_000, label = 'job' } = {}) {
  const home = `/home/${ctx.client.user}`;
  ...
  await remote.saveFile(ctx.client, JOBS, `${id}.sh`, wrapJob(id, body, home));
  ...
  return pollResult(ctx, id, { onProgress, timeout, label });
}

async function pollResult(ctx, id, { onProgress, timeout, label }) {
  const deadline = Date.now() + timeout;
  let lastStep = '';
  let waited = 0;

  while (Date.now() < deadline) {
    // Worker canlıyken iş 2 saniyede alınıyor; sık yoklamak mantıklı.
    const delay = waited < 30_000 ? 1500 : 3000;
    await sleep(delay);
    waited += delay;
