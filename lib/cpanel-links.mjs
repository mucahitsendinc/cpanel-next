...
/**
 * cPanel'in kendi web terminali.
 *
 * Aracın içindeki terminal çoğu iş için yeterli (bkz. shell/session.mjs) ama
 * iki durumda buna ihtiyaç var: gerçekten ETKİLEŞİMLİ bir program çalıştırmak
 * (vim, top, `composer` soru soruyorsa) ve bizim worker'ımız hiç ayağa
 * kalkmadığında bir çıkış yolu bırakmak.
 *
 * ⚠ Host bu özelliği kapatmış olabilir; Terminal cPanel'de ayrı bir yetki.
 * Bağlantı yine üretiliyor — açılmıyorsa kullanıcı bunu cPanel'de görüyor,
 * ve biz "kapalı" diye yanlış bir iddiada bulunmuyoruz.
 */
export function terminal(account, { theme = DEFAULT_THEME } = {}) {
  return deepLink(account, `/frontend/${theme || DEFAULT_THEME}/terminal/index.html`);
}
