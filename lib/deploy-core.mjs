...
import { buildProject, makeZip, pinToLockfile, ensureDependencies } from './packager.mjs';
...
import { normalizeHooks, hasHooks } from './hooks.mjs';
...
export async function runDeploy(ctx, spec, onEvent = () => {}) {
  const {
    ...
    excludes = [],
    /*
     * Sunucudaki ortam dosyaları temizlikten MUAF.
     *
     * Bunların hiçbiri pakete girmiyor (packager izin listesiyle dışarıda
     * tutuyor), dolayısıyla temizlikte silinirlerse bir daha geri gelmiyorlar:
     * kullanıcının sunucuda elle ya da `deploymanager db` ile yazdığı
     * `DATABASE_URL` her deploy'da yok oluyordu.
     */
    preserve = ['.env', '.env.local', '.env.production', '.env.production.local'],
    noBuild = false,
    transport = null,
    nodeVersion = null,
    cleanModules = false,
  ...
  /*
   * Hook tanımını burada doğruluyoruz — sürücüye ulaşmadan önce.
   *
   * Geçersiz bir tanım deploy'u DURDURMAZ (kullanıcının build'i çoktan koştu),
   * ama sessizce de geçmez: hangi satırın yok sayıldığı ekrana yazılır.
   */
  ...
  if (!noBuild) {
    ...
    // Bağımlılıklar yoksa kuruluyor: `next build` de tıpkı `vite build` gibi
    // `node_modules/.bin` olmadan "command not found" veriyor.
    ...
    await buildProject(cwd, { packageManager: project.packageManager, onOutput: output });
  } else {
  ...
  /*
   * Sürümleri lockfile'a sabitle — build ile çalıştırma AYNI sürümde olmalı.
   * Ayrıntılı gerekçe: packager.mjs → pinToLockfile.
   */
  const pin = pinToLockfile(cwd);
  const extraFiles = [{ path: 'tmp/restart.txt', content: stamp() }];
  if (pin) {
    extraFiles.push({ path: 'package.json', content: pin.content });
    emit('info', 'deploy.pinned', { count: pin.pinned.length, list: pin.pinned.join(', ') });
  }

  const pkg = await makeZip(cwd, {
    // Sabitlenmiş kopya extraFiles'tan gidiyor; özgün dosya pakete GİRMEMELİ,
    // yoksa zip'te aynı yol iki kez yer alır ve hangisinin açılacağı belirsiz.
    excludes: pin ? [...excludes, 'package.json'] : excludes,
    allowEnv: [],
  ...
  try {
    ...
    /*
     * Hızlı güncellemenin dayanağı: NE GÖNDERDİĞİMİZİN parmak izi.
     *
     * Bir sonraki güncelleme bu kaydı okuyup yerel dosyalarla karşılaştırıyor
     * ve yalnızca içeriği değişenleri yolluyor. `pkg.included` zaten elimizde
     * olduğu için proje ikinci kez yürünmüyor.
     *
     * Yazılamazsa yayın DÜŞMÜYOR — kayıp yalnızca bir sonraki güncellemenin
     * tam yayın olması.
     */
    ...
    if (ctx.driver.applyAll) {
      ...
      /*
       * Stok cPanel'de kabuk yolu yok — hook'lar KOŞMAZ.
       *
       * Bunu sessizce geçmek, `postInstall` içine migration yazmış birinin
       * migration'ın çalıştığını sanması demekti. README'de yazmak yetmiyor;
       * kullanıcı bu satırı deploy sırasında görmeli.
       */
      if (hasHooks(hooks)) emit('warn', 'deploy.hooksUnsupported', { count: hookCount });
