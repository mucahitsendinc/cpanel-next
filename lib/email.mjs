...
/**
 * Webmail'e ŞİFRE SORMADAN bağlanma oturumu.
 *
 * cPanel'in kendi arayüzünde posta kutusunun yanındaki "Check Email" düğmesi
 * tam olarak bunu yapıyor: hesabın sahibi olduğunuz bir posta kutusuna,
 * o kutunun şifresini bilmeden giriyorsunuz.
 *
 * ⚠ cPanel oturumunun aksine bu uç TOKEN İLE ÇALIŞIYOR — çünkü ürettiği şey
 * cPanel oturumu değil, webmail için tek kullanımlık bir oturum. Yani bu
 * özellik, kasada cPanel şifresi saklanmasa da çalışıyor.
 *
 * Dönen değerlerin kullanımı cPanel'in kendi belgesinde birebir yazılı:
 *   POST https://<hostname>:2096<token>/login
 *   gövde: session=<session>
 *
 * `hostname` null dönebiliyor; o durumda çağrıya cevap veren sunucu
 * kullanılıyor (belgenin kendi notu).
 */
export async function webmailSession(client, emailAddress) {
  const [login, domain] = splitEmail(emailAddress);
  const data = await client.uapi('Session', 'create_webmail_session_for_mail_user', {
    domain,
    login,
  });

  const session = pick(data, ['session'], null);
  const token = String(pick(data, ['token'], '') ?? '');
  if (!session) throw new UserError(t('mail.noWebmailSession'));

  const hostname = pick(data, ['hostname'], null) || client.host;
  return {
    // Belgedeki kalıp: token zaten `/cpsessNNNN` biçiminde, başına eğik çizgi
    // eklemek `//cpsess…` üretir ve 404 verir.
    url: `https://${hostname}:2096${token}/login`,
