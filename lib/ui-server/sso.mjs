import { fileManager, phpMyAdmin, terminal as cpanelTerminal, DEFAULT_THEME } from '../cpanel-links.mjs';
...
function gotoFor(target, { account, theme, dir, database }) {
  ...
  if (target === 'terminal') {
    /*
     * Host Terminal'i kapatmış olabilir — cPanel'de ayrı bir yetki. Bağlantı
     * yine de üretiliyor: kapalıysa kullanıcı bunu cPanel'de görüyor ve biz
     * doğrulayamadığımız bir şeyi "kapalı" diye iddia etmiyoruz.
     */
    return new URL(cpanelTerminal(account, { theme })).searchParams.get('goto_uri');
  }
