...
export async function handleApi(req, res, url, state) {
  ...
  try {
    ...
    if (route === 'profiles' && method === 'POST') {
      ...
      if (!body.master) throw new UserError('master password is required');
      ...
      process.env.CPANEL_NEXT_MASTER_PASSWORD = String(body.master);
      try {
      ...
      // Kasayı aynı ana şifreyle yeniden aç ki yeni profil hemen kullanılabilsin.
      const profiles = state.unlock(String(body.master));
    ...
    if (route === 'app-delete' && method === 'POST') {
      ...
      const removed = await ctx.driver.destroyApp(ctx, app, {
