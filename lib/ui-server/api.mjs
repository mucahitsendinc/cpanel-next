...
/**
 * Arayüzden gelen UZAK yolu ev dizinine göreli hâle getirir.
 *
 * ⚠ `remote.rel()` BUNU YAPMIYOR — yalnızca baştaki/sondaki eğik çizgileri
 * kırpıyor. `/home/bimtest` ona verildiğinde `home/bimtest` çıkıyor, cPanel
 * de bunu ev dizinine göreli sayıp `/home/bimtest/home/bimtest` arıyor.
 * Canlıda tam olarak bu hata görüldü:
 *
 *   Fileman::list_files: The directory "/home/bimtest/home/bimtest" does not exist
 *
 * Arayüz mutlak yollarla çalışıyor (kullanıcı nerede olduğunu görmeli), o
 * yüzden çeviri sunucu tarafında ve TEK yerde yapılıyor.
 */
function homeRel(ctx, value) {
  return toHomeRelative(value, ctx.client.user) ?? '';
}
...
async function fetchFromServer(ctx, { localDir, remoteDir, items }, emit) {
  ...
  for (const raw of items) {
    ...
    /*
     * KLASÖR: cPanel API'sinde dizin indirme yok, tek yol sunucuda arşiv
     * üretmek. Arşiv tek dosya sınırını aşarsa sunucuda parçalanıp
     * parçalar yerelde birleştiriliyor (bkz. download.mjs).
     */
    if (entry.type === 'dir') {
      say('step', 'fb.archiving', { name });
      const r = await downloadDirectory(ctx, `${remoteDir}/${name}`, localDir, {
        onProgress: (p) => emit({ type: p.step ? 'remote' : 'progress', text: p.step, ...p }),
      });
      if (r.parts > 1) say('info', 'fb.joined', { parts: r.parts });
      done += 1;
      continue;
    }
...
export async function handleApi(req, res, url, state) {
  ...
  try {
     ...
     */
    /**
     * `.deployignore` — oku ve yaz.
     *
     * Projenin kökünde duruyor, `.gitignore` söz dizimiyle. Yayın hattı,
     * hızlı güncelleme ve özet ekranı hepsi aynı dosyayı okuyor
     * (bkz. deployignore.mjs), yani burada kaydedilen desen her yerde geçerli.
     */
    if (route === 'deployignore' && method === 'POST') {
      const body = await readJson(req);
      ...
      if (body.content === undefined) {
        let content = '';
        try { content = fs.readFileSync(file, 'utf8'); } catch { /* yok */ }
        const { patterns, warnings } = parseDeployIgnore(content);
        return json(200, { content, count: patterns.length, warnings });
      }

      const content = String(body.content ?? '');
      // Kaydetmeden ÖNCE ayrıştırılıyor: desteklenmeyen bir satır (ör. `!ters`)
      // sessizce yok sayılmasın, kullanıcı hemen görsün.
      const { patterns, warnings } = parseDeployIgnore(content);
      if (content.trim()) fs.writeFileSync(file, content.endsWith('\n') ? content : `${content}\n`);
      else fs.rmSync(file, { force: true });

      return json(200, { saved: true, count: patterns.length, warnings });
    }
    ...
    if (route === 'remote-browse' && method === 'GET') {
      ...
      const rel = homeRel(ctx, url.searchParams.get('path') || home);
    ...
    if (route === 'transfer' && method === 'POST') {
      ...
      const remoteDir = homeRel(ctx, String(body.remoteDir ?? ''));
    ...
    if (route === 'remote-delete' && method === 'POST') {
      const body = await readJson(req);
      const ctx = await state.session(body.profile);
      const dir = homeRel(ctx, String(body.remoteDir ?? ''));
    ...
    if (route === 'remote-file' && method === 'GET') {
      const ctx = await state.session(url.searchParams.get('profile'));
      const raw = homeRel(ctx, url.searchParams.get('path') ?? '');
    ...
    if (route === 'remote-file' && method === 'POST') {
      const body = await readJson(req);
      const ctx = await state.session(body.profile);
      const raw = homeRel(ctx, String(body.path ?? ''));
      const dir = path.posix.dirname(raw);
      const file = path.posix.basename(raw);
      const content = String(body.content ?? '');
