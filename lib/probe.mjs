...
export async function probe(client, { refresh = false, verbose = false } = {}) {
  ...
  try {
    ...
    /*
     * Tema, cPanel'in kendi ekranlarına derin bağlantı kurabilmek için gerekli:
     * Dosya Yöneticisi'nin yolu `/frontend/<tema>/filemanager/…`. Sabit
     * `jupiter` yazmak, temayı değiştirmiş hesaplarda bağlantıyı 404 yapardı.
     */
    if (info?.theme) result.theme = String(info.theme);
    if (info?.home) result.home = String(info.home);
  } catch {
    /* önemsiz */
  }
