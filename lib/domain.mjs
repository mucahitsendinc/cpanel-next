...
/**
 * Belge kökünü EV DİZİNİNE GÖRELİ hâle getirir.
 *
 * ⚠ cPanel `documentroot` alanını MUTLAK yol olarak döndürüyor
 * (`/home/bimtest/tests.example.com`). Aracın geri kalanı yolları ev dizinine
 * göreli bekliyor ve bu uyuşmazlık iki gerçek arızaya yol açtı:
 *
 *   · Laravel yayını `/home/bimtest/home/bimtest/tests.example.com` adresine
 *     yazmaya çalıştı; cPanel "The file '' does not exist" dedi ve bakım
 *     sayfası kurulamadı.
 *   · `assertAppRoot`'un BELGE KÖKÜ ÇAKIŞMA denetimi hiç eşleşmiyordu:
 *     `rel('/home/u/site.com')` = `home/u/site.com`, kullanıcının yazdığı ad
 *     ise `site.com`. Yani kaynak kodu yayına açık bir dizine koymayı
 *     engelleyen kontrol sessizce devre dışıydı.
 *
 * Ev dizininin dışındaki bir yol gelirse dokunmuyoruz — yalnızca baştaki ve
 * sondaki eğik çizgiler kırpılıyor (eski davranış).
 */
export function toHomeRelative(value, user) {
  const s = String(value ?? '').trim();
  if (!s) return null;
  const home = `/home/${user}`;
  if (s === home) return '';
  if (s.startsWith(`${home}/`)) return s.slice(home.length + 1).replace(/\/+$/, '');
  return s.replace(/^\/+|\/+$/g, '');
}
...
export async function listDomains(client) {
  ...
  try {
    ...
    if (domains.length) return normalizeDocroots(dedupe(domains), client.user);
  } catch {
  ...
  return normalizeDocroots(dedupe(domains), client.user);
}

/** Her domainin belge kökünü ev dizinine göreli hâle getirir. */
function normalizeDocroots(domains, user) {
  for (const d of domains) {
    if (d.docroot) d.docroot = toHomeRelative(d.docroot, user);
  }
  return domains;
}
