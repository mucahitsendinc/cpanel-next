...
/*
 * Başlık şeridi — sürüm HER komutta görünüyor.
 *
 * Önceden sürümü öğrenmenin tek yolu `deploymanager --version` idi; bir
 * hatayı bildirirken ilk sorulan şey olduğu için her ekranda durması
 * gerekiyor. Web arayüzü ve masaüstü de aynı değeri gösteriyor
 * (bkz. version.mjs — tek kaynak).
 */
export const intro = (msg) =>
  p.intro(`${color.bgCyan(color.black(` ${msg} `))} ${color.dim(`v${VERSION}`)}`);
