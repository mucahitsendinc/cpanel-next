...
export async function run({ flags, cwd }) {
  ...
  /*
   * Laravel güncellemesi ayrı hattan gidiyor.
   *
   * Node rejimi tespiti (probe) burada anlamsız ve YAPILMAMALI: sürücü
   * bulunamazsa `deploy.noDriver` ile duruyoruz, oysa Laravel'in sürücüye
   * ihtiyacı yok. PHP-only bir hesapta güncelleme bu yüzden hiç başlamazdı.
   */
  if (project.framework === 'laravel') {
    ...
    const { updateLaravel } = await import('./deploy-laravel.mjs');
    return updateLaravel(ctx, { target, appRoot });
  }
