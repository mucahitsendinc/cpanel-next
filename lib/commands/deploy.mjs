...
import { assertAppRoot, assertOwnership, isDestructive } from '../guards.mjs';
...
async function deploy(ctx) {
  ...
  if (choice.mode === 'update' || dirExists) {
    await withSessionFallback(ctx, () =>
      assertOwnership(ctx.client, appRoot, { adopt: flags.adopt, dirExists })
    );
  }
