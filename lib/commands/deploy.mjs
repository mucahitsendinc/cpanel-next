...
async function deploy(ctx) {
  ...
  /*
   * Laravel buradan AYRILIYOR.
   *
   * Node rejimi tespiti (Passenger / CloudLinux Selector) Laravel için
   * anlamsız: PHP'yi sunucunun kendi işleyicisi çalıştırıyor, sürücü diye bir
   * şey yok. Sonda değil burada dallanmak, aşağıdaki her adımın "Laravel ise
   * atla" ile kirlenmesini engelliyor.
   */
  if (project.framework === 'laravel') {
    const { deployLaravel } = await import('./deploy-laravel.mjs');
    return deployLaravel(ctx);
  }
  ...
  /* -- 7. Paket planı ---------------------------------------------------- */
  // `.deployignore` + `.cpanel-next.json:exclude` — bkz. deployignore.mjs.
  const ignore = resolveExcludes(cwd, { projectExclude: cfg.project?.exclude ?? null });
  for (const w of ignore.warnings) log.warn(w);
  const excludes = ignore.excludes;
  ...
  let result;
  // Ctrl+C artık süreci öldürmüyor: sunucudaki işe iptal bayrağı bırakıyor ve
  // iş bir sonraki ADIM SINIRINDA duruyor. Ortasından kesmek yarım kurulmuş
  // bir uygulama bırakırdı.
  let jobId = null;
  const onSigint = () => {
    if (!jobId) return;
    s2.message(t('worker.cancelling'));
  ...
  try {
    ...
    s2.stop(err?.cancelled ? t('worker.cancelled', { label: t('deploy.title') }) : t('deploy.remoteFailed'), 1);
    throw err;
  } finally {
