...
export async function applyAll(ctx, spec) {
  ...
  /*
   * Bu da `hookLine`'dan geçiyor. Eskiden düz `sh -c` idi ve venv'in bin
   * dizini PATH'te olmuyordu — yani `preInstall` içinde `node`/`npx` yazmak
   * ya sistem sürümünü buluyor ya da hiç bulamıyordu, `postInstall` içinde
   * aynı komut çalışırken. Aynı dosyada iki farklı kural olmaz.
   *
   * `create` bu noktadan önce koştuğu için venv KESİNLİKLE var; yeni
   * uygulamada bile PATH doğru.
   */
  for (const cmd of hooks.preInstall ?? []) {
    push(55, `Hook (kurulum oncesi): ${short(cmd)}`, hookLine(cmd, home, foundRoot, version));
  }
