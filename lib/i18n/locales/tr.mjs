...
`,
  ...
  update: {
    ...
    frameworkMismatch: '~/{appRoot} klasörüne {expected} yayınlanmış ama seçilen proje {got}.',
    frameworkMismatchHint: 'Yanlış proje seçili. Bu klasörün bağlı olduğu proje: {project}',
  },
  ...
  download: {
    ...
    archiving: 'Sunucuda arşiv oluşturuluyor',
  ...
  laravel: {
    ...
    recovering: 'Site 500 verdi — önbellek temizlenip yeniden deneniyor…',
    recovered: 'Önbellek temizlendi, site açıldı ({status}). config:cache kaldırıldığı için site bir tık yavaş; sebebini yukarıdaki logdan araştırın.',
    recoverFailed: 'Önbellek temizliği de düzeltmedi ({status}). Yukarıdaki log hatanın kendisi.',
    unhealthy: 'Yayın bitti ama site {status} veriyor. Aşağıdaki hata laravel.log’dan:',
    logTooBig: 'laravel.log çok büyük, buraya getirilmedi — sunucuda bakın.',
    noLog: 'laravel.log okunamadı ya da boş. Hatayı görmek için .env içinde APP_DEBUG=true yapıp siteyi bir kez açın.',
    ...
    sClean: 'Temizlik',
    sCleanOn: 'önbellek boşaltılacak, loglar sıfırlanacak',
    sCleanOff: 'yapılmayacak',
    sDb: 'Veritabanı',
  ...
  shell: {
    empty: 'Komut boş.',
    busy: 'Önceki komut hâlâ çalışıyor.',
  },

  quick: {
    comparing: 'Değişiklikler karşılaştırılıyor',
    plan: '{changed} değişmiş, {added} yeni, {removed} silinmiş · {same} dosya aynı, gönderilmiyor',
    removing: 'Sunucudan silinen {count} dosya',
    noChanges: 'Değişiklik yok — gönderilecek bir şey bulunamadı.',
    done: 'Hızlı güncelleme tamam: {changed} değişmiş, {added} yeni, {removed} silinmiş.',
    noManifest: 'Bu klasör için dosya kaydı yok.',
    noManifestHint: 'Hızlı güncelleme, en son neyi gönderdiğimizin kaydına dayanıyor. Bir kez normal güncelleme yapın; sonrakiler hızlı olur.',
    lockChanged: 'composer.lock değişmiş.',
    lockChangedHint: 'Bağımlılıklar değiştiğinde kurulum gerekiyor ve bu tam güncellemenin işi. Bir kez normal güncelleme yapın.',
  },

  fb: {
    nothing: 'Aktarılacak bir şey seçilmedi.',
    oneAtATime: 'Silme tek seferde tek öğe için yapılıyor.',
    confirmName: 'Silmek için "{name}" yazmanız gerekiyor.',
    deleteFailed: 'Silinemedi: {list}',
    notFound: 'Sunucuda bulunamadı: {name}',
    dirDownload: '{name} bir klasör — klasör indirme henüz yok, içindeki dosyaları seçin.',
    unsafeName: 'Güvenli olmayan dosya adı atlandı: {name}',
    archiving: 'Klasör arşivleniyor: {name}',
    joined: '{parts} parça indirilip birleştirildi.',
    downloading: 'İndiriliyor: {name}',
    fetched: '{count} dosya indirildi → {dir}',
    sent: '{count} dosya gönderildi.',
    skippedDir: 'Araç klasörü atlandı: {name}',
    skippedLinks: '{count} sembolik bağ atlandı.',
    truncated: 'Çok fazla dosya — ilk {max} tanesi gönderiliyor.',
    tooBigEdit: 'Dosya düzenleyici için çok büyük ({max} KB üstü). cPanel Dosya Yöneticisi’ni kullanın.',
    binary: 'Bu bir metin dosyası değil; düzenleyicide açmak onu bozardı.',
    changed: 'Dosya siz açtıktan sonra sunucuda değişmiş. Kaydetmedik — önce yeniden açıp bakın.',
  },
  ...
  vault: {
    noVault: 'Bu makinede kurulmuş bir kasa yok.',
    noVaultHint: 'Önce bir cPanel hesabı ekleyin; kasa o sırada kuruluyor.',
  ...
  packager: {
    ...
    noBuildOutput: 'Build çıktısı oluşmadı ({files}) — betik bir şey üretmedi.',
    installing: 'Bağımlılıklar kuruluyor ({pm} install)',
  ...
  deploy: {
    ...
    hooksPlanned: '{count} özel komut çalıştırılacak.',
    projectMissing: 'Proje klasörü bulunamadı: {dir}',
    hooksUnsupported:
