export default {
  ...
  cli: {
    ...
    help: `
  cpanel-next — deploy Next.js and Laravel projects to cPanel
  ...
  COMMANDS
    ...
    update            Update the linked project in one command (asks nothing)
    ...
    db                MySQL: list | create <name> | drop <name> | users | pma
  ...
  OPTIONS
    ...
    --driver <name>          passenger | cloudlinux  (overrides detection)

    --env-local              (db create) write DATABASE_URL to local .env.local
    --app-root <folder>      (db create) write DATABASE_URL to .env on the server

    --migrate <mode>         (Laravel) none | migrate | migrate-seed | fresh-seed
    --no-migrate             (Laravel) do not run any migration
    --vendor <mode>          (Laravel) auto | always | server
    --keep-debug             (Laravel) leave APP_DEBUG on the server alone
...
`,
  ...
  maintenance: {
    ...
    htaccessUnreadable: 'Could not read {dir}/.htaccess; the maintenance page was not installed rather than overwrite it.',
    labelDomain: 'Domain',
    labelDocroot: 'Document root',
  },

  update: {
    title: 'cpanel-next · update',
    notLinked: 'This project is not linked to a domain yet.',
    notLinkedHint: 'Deploy once with "deploymanager"; after that updates are a single command.',
    checking: 'Verifying the link',
    domainGone: '"{domain}" is no longer on this account.',
    folderGone: 'The folder ~/{appRoot} is not on the server.',
    noMarker: '~/{appRoot} does not look like it was deployed with this tool.',
    otherProject: 'This folder was deployed from another project: {project}',
    confirmAnyway: 'Update anyway?',
    frameworkMismatch: '~/{appRoot} holds a {expected} deployment, but the selected project is {got}.',
    frameworkMismatchHint: 'The wrong project is selected. This folder is linked to: {project}',
  },

  subdomain: {
    boundToApp: 'This subdomain is bound to the application "{app}". Remove the application first.',
  },

  db: {
    title: 'cpanel-next · database',
    reading: 'Reading databases',
    count: '{count} databases',
    empty: 'No databases on this account.',
    headers: ['Database', 'Size', 'Users'],
    userHeaders: ['User', 'Databases'],
    server: 'MySQL server: {host}:{port}',
    prefix: 'Name prefix: {prefix}',
    noPrefix: 'No name prefix (disabled by the host).',
    creating: 'Provisioning the database',
    created: 'Ready: {database}',
    dbExisted: 'The database already existed and was left alone: {database}',
    userExisted: 'The user already existed; its password was NOT changed: {user}',
    passwordOnce: 'This password will not be shown again — save it now.',
    connection: 'Connection string',
    dropping: 'Dropping the database',
    dropped: 'Dropped: {database}',
    deleteHint: 'Dropping a database has no backup; you must type its name exactly.',
    invalidName: 'Invalid name: {name}',
    invalidNameHint: 'Only letters, digits and underscores are allowed (a cPanel rule).',
    nameTooLong: '"{name}" is too long — at most {max} characters for a {kind}.',
    nameRequired: 'Give a database name: deploymanager db create <name>',
    namePrompt: 'Database name (the prefix is added automatically)',
    credentials: 'Credentials',
    passwordLabel: 'Password',
    pmaHint: 'phpMyAdmin asks for your cPanel login; your password never passes through this tool.',
    envWritten: '{file} updated: {keys}',
    unknownSub: 'Unknown subcommand: {name}',
    subHint: 'Available: list · create · drop · users · pma',
  },

  sso: {
    locked: 'The vault is locked — enter your master password in the interface first.',
    noProfile: 'No such account: {name}',
    noPassword:
      'This account has no stored password; redirecting you to the cPanel login page.',
    signingIn: 'Signing in to {host}…',
    openingWebmail: 'Opening webmail for {email}…',
    storePassword: 'Store my cPanel password in the vault',
    storePasswordHint:
      'phpMyAdmin and File Manager then open in one click without asking for a password. ' +
      'It is encrypted with your master password (AES-256-GCM) in a 0600 file. ' +
      'With this off, those screens redirect to the cPanel login instead.',
  },

  mail: {
    emailRequired: 'An address and a domain are required.',
    invalidEmail: 'Invalid email address: {email}',
    invalidLocal: 'Invalid address: {name}',
    invalidLocalHint: 'Only letters, digits, dots, hyphens and underscores are allowed.',
    noWebmailSession: 'Could not create a webmail session.',
    deleteHint: 'Deleting a mailbox deletes every message in it; you must type the address exactly.',
  },

  ftp: {
    userRequired: 'A username is required.',
    invalidUser: 'Invalid username: {name}',
    invalidUserHint: 'It must start with a letter or digit; dots, hyphens and underscores are allowed.',
    deleteHint: 'Deleting an FTP account does not delete files; you must type the username exactly.',
  },

  account: {
    bothPasswords: 'Both the current and the new password are required.',
    samePassword: 'The new password is the same as the current one.',
  },

  download: {
    title: 'cpanel-next · download',
    notFound: 'Not found on the server: ~/{path}',
    pathRequired: 'No path to download was given.',
    tooLarge: 'The file is too large ({size} bytes, limit {max}).',
    tooLargeHint: 'cPanel has no download endpoint in its API, so large files are moved in chunks and take minutes at this size. Use cPanel’s own File Manager.',
    noParts: 'No prepared parts were found on the server.',
    partFailed: 'A part could not be read: {part}',
    sizeMismatch: 'The downloaded file is incomplete: expected {expected} bytes, got {got}.',
    notOnDisk: 'The file is not on disk: {path}',
    archiving: 'Creating the archive on the server',
    archiveFailed: 'Could not create the archive',
  },

  dbbackup: {
    title: 'cpanel-next · database backup',
    noMysqldump: 'mysqldump not found',
    dumpFailed: 'mysqldump failed',
    empty: 'The backup file came out empty',
  },
  ...
  laravel: {
    ...
    noDocroot: 'No document root found for "{domain}".',
    missingArtisan: 'The package was extracted but there is no artisan file — the upload is incomplete.',
    htaccess: 'Writing the rewrite rule (.htaccess)',
    running: 'Running setup steps on the server',
    buildingAssets: 'Building assets ({builder})',
    assetsPrebuilt: 'No local node_modules but public/build is present — skipping the build.',
    settingInvalid: 'Invalid Laravel setting, fell back to the default: {detail}',

    vendor: {
      first: 'First install: shipping vendor.',
      changed: 'composer.lock changed: shipping vendor.',
      unchanged: 'composer.lock unchanged: NOT shipping vendor, keeping the one on the server.',
      always: 'vendor mode "always": shipped on every deploy.',
      server: 'vendor mode "server": composer will run on the server.',
      unknown: 'No composer.lock digest to compare against: shipping vendor.',
    },
    vendorMode: {
      auto: 'auto (ship when the lock changes)',
      always: 'always (ship every time)',
      server: 'server (composer runs on the server)',
    },
    vendorMissing:
      'No vendor/autoload.php. It was not shipped and is not on the server — ' +
      'set the vendor mode to "always" or "server".',

    migrateMode: {
      none: 'will not run',
      migrate: 'migrate --force',
      'migrate-seed': 'migrate --force + db:seed',
      'fresh-seed': 'migrate:fresh --seed  (DROPS EVERY TABLE)',
    },
    freshWarning:
      'migrate:fresh DROPS every table and rebuilds it. Existing data is lost. ' +
      'For another mode: --migrate migrate',

    noEnvOnServer: 'There is no .env on the server and no .env.example in the package.',
    noEnvOnServerHint:
      'Add a .env.example to your project, or place a .env in the folder on the server by hand.',
    envCreated: 'No .env on the server; created it from .env.example.',
    envPatched: 'Updated .env on the server: {keys}',
    pruned: 'Pruned {count} stale files under public/.',
    manifestTooBig:
      'More than {limit} files under public/: stale-file pruning was skipped.',

    exposed: 'WARNING — readable from the internet: {list}',
    notExposed: 'Source files are not readable from the internet (verified).',
    recovering: 'The site returned 500 — clearing caches and retrying…',
    recovered: 'Caches cleared and the site came up ({status}). It runs slightly slower without config:cache; use the log above to find the cause.',
    recoverFailed: 'Clearing the caches did not fix it ({status}). The log above is the actual error.',
    unhealthy: 'The release finished but the site returns {status}. The error below is from laravel.log:',
    logTooBig: 'laravel.log is too large to fetch here — look at it on the server.',
    noLog: 'laravel.log is empty or unreadable. To see the error, set APP_DEBUG=true in .env and load the site once.',
    exposedTitle: 'SOURCE FILES ARE EXPOSED',
    exposedBody:
      'These URLs are readable from the internet: {list}\n' +
      'Your .env contains your database password and application key.',
    exposedFix:
      'The cause is almost always .htaccess being ignored (AllowOverride None) or ' +
      'mod_rewrite being disabled. Ask your host; do not leave this domain live until it is fixed.',

    dbUnavailable: 'Could not list databases',
    askDb: 'Database',
    dbNew: 'Create a new database',
    dbNewHint: 'Database + user + privileges; .env written automatically',
    dbExisting: 'Use an existing database',
    dbExistingHint: 'Pick from the list',
    dbSkip: 'Leave it alone',
    dbSkipHint: 'The .env on the server stays as it is',
    pickDb: 'Which database?',
    askDbUser: 'Which user should connect to it?',
    dbUserNew: 'Create a new user',
    dbUserNewHint: 'The tool generates the password and writes it to .env',
    dbUserExisting: 'Existing user',
    pickDbUser: 'Which user?',
    dbPasswordPrompt: 'That user’s password (cPanel does not show it, and neither do we)',

    folderNotEmpty:
      '~/{appRoot} is not empty ({count} entries). Code directories will be deleted and reinstalled.',
    sRoot: 'Application folder',
    sNotEmpty: '  ({count} entries)',
    sPublic: 'Served from',
    sViaHtaccess: '(routed via .htaccess)',
    sMode: 'Mode',
    sFirst: 'FIRST INSTALL',
    sUpdate: 'Update',
    sMigrate: 'Migration',
    sVendor: 'vendor',
    sDebug: 'APP_DEBUG',
    sDebugOff: 'will be set to false',
    sDebugKeep: 'left alone',
    sClean: 'Cleanup',
    sCleanOn: 'cache flushed, logs truncated',
    sCleanOff: 'skipped',
    sDb: 'Database',
    sDbUntouched: 'the .env on the server is kept',
  },

  shell: {
    empty: 'Empty command.',
    busy: 'The previous command is still running.',
  },

  quick: {
    comparing: 'Comparing changes',
    plan: '{changed} changed, {added} new, {removed} deleted · {same} files identical, not sent',
    removing: 'Removing {count} files from the server',
    noChanges: 'Nothing changed — there is nothing to send.',
    done: 'Quick update done: {changed} changed, {added} new, {removed} deleted.',
    noManifest: 'There is no file record for this folder.',
    noManifestHint: 'Quick update relies on a record of what we last sent. Run one normal update; the ones after that will be quick.',
    lockChanged: 'composer.lock has changed.',
    lockChangedHint: 'Changed dependencies need an install, and that is a full update. Run one normal update.',
  },

  fb: {
    nothing: 'Nothing selected to transfer.',
    oneAtATime: 'Deletion is done one item at a time.',
    confirmName: 'Type "{name}" to delete it.',
    deleteFailed: 'Could not delete: {list}',
    notFound: 'Not found on the server: {name}',
    dirDownload: '{name} is a folder — folder download is not available yet, select the files inside.',
    unsafeName: 'Skipped an unsafe file name: {name}',
    archiving: 'Archiving folder: {name}',
    joined: 'Downloaded and joined {parts} parts.',
    downloading: 'Downloading: {name}',
    fetched: '{count} files downloaded → {dir}',
    sent: '{count} files sent.',
    skippedDir: 'Skipped a tool folder: {name}',
    skippedLinks: 'Skipped {count} symbolic links.',
    truncated: 'Too many files — sending the first {max}.',
    tooBigEdit: 'Too large for the editor (over {max} KB). Use the cPanel File Manager.',
    binary: 'This is not a text file; opening it in the editor would corrupt it.',
    changed: 'The file changed on the server after you opened it. Nothing was saved — reopen and check first.',
  },
  ...
  vault: {
    noVault: 'There is no vault set up on this machine.',
    noVaultHint: 'Add a cPanel account first; the vault is created then.',
  ...
  detect: {
    noPackageJson: 'Neither package.json nor artisan here — this does not look like a supported project.',
    packageJsonUnreadable: 'Could not read package.json: {error}',
    noComposerLock:
      'No composer.lock. Dependency versions cannot be pinned; run "composer install" first.',
    noPublicIndex: 'No public/index.php — this does not look like a Laravel project.',
    noVendor:
      'No local vendor/. The default mode ships your local vendor; run ' +
      '"composer install --no-dev -o" or set the vendor mode to "server".',
    noEnvExample:
      'No .env.example. If the server has no .env there is nothing to base a new one on.',
    noAssetBuild:
      '{builder} is configured but public/build is missing — build your assets locally (npm run build).',
  ...
  packager: {
    ...
    noBuildOutput: 'No build output appeared ({files}) — the script produced nothing.',
    installing: 'Installing dependencies ({pm} install)',
  ...
  worker: {
    ...
    cancelling: 'Cancelling (will stop at the next step)…',
    waiting: 'Waiting for the server listener (up to 1 min)',
  },
  ...
  apps: {
    ...
    deletingApp: 'Removing {name}…',
    deleteLabel: 'Application removal',
  ...
  deploy: {
    ...
    pinned: 'Versions pinned to the lockfile ({count}): {list}',
    hooksPlanned: '{count} custom command(s) will run.',
    projectMissing: 'Project folder not found: {dir}',
    hookInvalid: 'Invalid hook definition, ignored: {detail}',
    hooksUnsupported:
      'No shell path on this server (stock cPanel): {count} hook(s) did NOT run.',
