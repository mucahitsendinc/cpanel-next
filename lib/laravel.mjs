 ...
 *
 * ⚠ BAKIM SAYFASI YÖNLENDİRMENİN DIŞINDA — İLK KURAL BU.
 *
 * Bakım kuralı `ErrorDocument 503 /cpanel-next-maintenance.html` diyor ve
 * Apache bunun için İÇ İSTEK açıyor. O iç istek de `.htaccess`'ten geçiyor;
 * hiçbir şey engellemezse buradaki kural onu `public/` içine yeniden yazıyor,
 * dosya orada olmadığı için sayfa bulunamıyor ve kullanıcı bizim hazırladığımız
 * sayfa yerine Apache'nin çıplak "Service Unavailable" metnini görüyor.
 *
 * Yani bakım sayfası, tam da gösterilmesi gereken anda kayboluyordu. Bir
 * satırlık erken çıkış bunu kesiyor. `.php` de kapsanıyor: bakım kuralı, PHP
 * koşuyorsa 503 durum kodunu koruyabilmek için `.php` sürümünü seçiyor.
 */
...
RewriteEngine On
RewriteRule ^cpanel-next-maintenance\\.(html|php)$ - [L]
...
export function artisanPlan({
  ...
  /*
   * YAYIN SONRASI TEMİZLİK.
   *
   * ⚠ BURADA `optimize:clear` KOŞMUYOR — `optimize` açıkken.
   *
   * İstenen şey "yayından sonra önbellekler temizlensin"; ama tam bir
   * `optimize:clear` bir satır önce KURULAN `config:cache`/`route:cache`/
   * `view:cache`'i silerdi. Yani her deploy, önbelleği kurup hemen ardından
   * çöpe atardı ve site kalıcı olarak önbelleksiz koşardı.
   *
   * Bu yüzden ikiye ayrıldı:
   *
   *   · `optimize` AÇIK  → yalnızca `cache:clear`. Bu UYGULAMA (veri)
   *     önbelleği; `Cache::` ile yazılanlar. Eski kodun bıraktığı veri
   *     gidiyor, üretim önbelleği duruyor. Zaten istenen buydu.
   *
   *   · `optimize` KAPALI → kurulan bir önbellek yok, tam `optimize:clear`
   *     bedava. Config, route, view, event ve derlenmiş sınıflar dahil.
   *
   * Şema önbelleği migration'dan ÖNCE temizlendi (yukarıya bakın); buradaki
   * adım ondan bağımsız.
   */
  if (clean) {
    steps.push(
      optimize
        ? { label: 'cache:clear', cmd: `${artisan} cache:clear`, fatal: false }
        : { label: 'optimize:clear', cmd: `${artisan} optimize:clear`, fatal: false },
    );

    /*
     * Loglar SIFIRLANIYOR, SİLİNMİYOR.
     *
     * `rm` üç şeyi bozuyor:
     *
     *   1. PHP-FPM dosyayı açık tutuyorsa `rm` yalnızca dizin kaydını siler;
     *      süreç bağlantısı kopmuş inode'a yazmaya devam eder ve loglar
     *      sessizce KAYBOLUR — dosya "temiz" görünür, aslında yok.
     *   2. Dosyayı bir sonraki isteğin yeniden oluşturması gerekir; o istek
     *      web sunucusu kullanıcısıysa sahiplik değişir.
     *   3. Kip bitleri gider; `chmod` adımı çoktan koşmuş durumda.
     *
     * `: > dosya` inode'u koruyor, boyutu sıfırlıyor. Sahip ve izin aynı
     * kalıyor, açık tanıtıcılar geçerli kalıyor.
     *
     * Günlük kanal (`daily`) `laravel-YYYY-AA-GG.log` üretiyor, o yüzden tek
     * bir isim değil `*.log` taranıyor. `-maxdepth 1`: alt dizinlere inmiyor.
     * `-exec … +` toplu çağırıyor, dosya başına süreç açmıyor.
     */
    steps.push({
      label: 'log temizliği',
      cmd: '{ find storage/logs -maxdepth 1 -type f -name "*.log" ' +
        '-exec sh -c \'for f; do : > "$f"; done\' _ {} + 2>/dev/null || :; }',
      fatal: false,
    });
  }
...
export function normalizeSettings(raw = {}, { first = false } = {}) {
  ...
  /*
   * ⚠ VARSAYILANLAR VERİTABANINA DOKUNMAMA YÖNÜNDE.
   *
   * İlk hâli güncellemede `migrate`, ilk kurulumda `fresh-seed` diyordu ve
   * ikisi de yanlıştı:
   *
   *   · GÜNCELLEME. Panelden "Güncelle"ye basmak tek tık; o tıkla şemayı
   *     değiştiren bir komut çalıştırmak, kullanıcının istemediği bir kararı
   *     onun adına vermek. Migration artık AÇIKÇA istenmedikçe koşmuyor.
   *
   *   · İLK KURULUM. `migrate:fresh` BÜTÜN TABLOLARI SİLİYOR. Boş bir
   *     veritabanında zararsız, ama kullanıcı VAR OLAN bir veritabanını
   *     seçtiyse verisini siliyor — hem de varsayılan davranış olarak.
   *     `migrate-seed` şemayı kuruyor, hiçbir şeyi düşürmüyor. `fresh-seed`
   *     hâlâ seçilebilir ama artık asla varsayılan değil.
   *
   * Her iki kip de `.cpanel-next.json` ile ya da ekrandan değiştirilebiliyor;
   * amaç seçeneği kaldırmak değil, sessiz kararı kaldırmak.
   */
  return {
    migrate: first
      ? pick(raw.firstMigrate, MIGRATE_MODES, 'migrate-seed', 'firstMigrate')
      : pick(raw.migrate, MIGRATE_MODES, 'none', 'migrate'),
    ...
    /*
     * Yayın sonrası temizlik — önbellek ve loglar. Varsayılan AÇIK: veri
     * kaybettirmiyor (log sıfırlıyor, şemaya dokunmuyor), o yüzden `migrate`
     * gibi sessiz-karar riski taşımıyor.
     */
