...
export function buildEnvPatch({
  ...
  if (db) {
    ...
    patch.DB_HOST = db.host === 'localhost' ? '127.0.0.1' : db.host;
...
export function artisanPlan({
  ...
  // Yazma izinleri: Laravel bu iki dizine yazamazsa ilk istekte 500 veriyor
  // ve hata kayıtlara bile düşemiyor.
  steps.push({
    label: 'izinler',
    cmd: 'mkdir -p storage/framework/cache storage/framework/sessions storage/framework/views ' +
      'storage/logs bootstrap/cache && chmod -R 775 storage bootstrap/cache',
    fatal: true,
  });
  ...
  /*
   * Önbellek: önce TEMİZLE, sonra kur.
   *
   * Eski `config.cache` dosyası yeni `.env` ile uyuşmuyorsa uygulama eski
   * veritabanı bilgileriyle açılmaya çalışır — ve hata, .env'i doğru yazmış
   * olan kullanıcıyı tamamen yanlış yere bakmaya götürür.
   */
