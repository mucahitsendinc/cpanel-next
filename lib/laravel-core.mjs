...
export async function runLaravelDeploy(ctx, spec, onEvent = () => {}) {
  ...
  /*
   * Hariç tutma listesi TEK KAYNAKTAN: aracın Laravel listesi + projenin
   * `.deployignore` dosyası + `.cpanel-next.json` içindeki `exclude`.
   * Bkz. deployignore.mjs — bu üçünün ayrı ayrı kurulması, ayarın bazı
   * yollarda sessizce yok sayılmasına yol açıyordu.
   */
  const ignore = resolveExcludes(cwd, {
    base: [...LARAVEL_EXCLUDES, ...(vendor.ship ? [] : ['vendor/**'])],
    // ⚠ `project` (detectProject) ayar dosyasını TAŞIMIYOR — buradan okunuyor.
    // İlk yazımda `project.config.exclude` yazmıştım; öyle bir alan olmadığı
    // için değer hep undefined kalıyor ve düzeltmeye çalıştığım kusurun
    // aynısı Laravel yolunda yeniden doğuyordu.
    projectExclude: loadProjectConfig(cwd)?.exclude ?? null,
  });
  for (const w of ignore.warnings) emit('warn', 'deploy.hookInvalid', { detail: w });
