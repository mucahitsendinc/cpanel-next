...
export async function runLaravelDeploy(ctx, spec, onEvent = () => {}) {
  ...
  if (settings.buildAssets && project.assetBuilder && hasBuildScript(cwd)) {
    ...
    const hasOutput = fs.existsSync(path.join(cwd, 'public', 'build'));
    ...
    if (hasModules) {
      ...
      emit('warn', 'laravel.assetsPrebuilt', {});
    } else {
      throw new UserError(
        t('laravel.noNodeModules', { builder: project.assetBuilder }),
        t('laravel.noNodeModulesHint', { pm: project.packageManager })
      );
    }
  }
