 ...
 */
export function assertAppRoot(appRoot, { docroots = [], force = false, framework = null } = {}) {
  ...
  /*
   * ⚠ LARAVEL'DE BELGE KÖKÜ ZORUNLU DÜZEN.
   *
   * Bu denetim Next.js için doğru: kaynak kodu yayına açılan bir dizine
   * koymak `.env` dosyasını internete açar. Laravel'de ise uygulama belge
   * kökünün İÇİNE kuruluyor ve `.htaccess` isteği `public/` alt dizinine
   * yönlendiriyor — kaynak dizinleri URL'den erişilemez oluyor (bkz.
   * lib/laravel.mjs → HT_BLOCK) ve yayından sonra bu gerçekten ölçülüyor
   * (laravel-core.mjs → checkExposure).
   *
   * Yani Laravel için bu bir hata değil, tek geçerli düzen. Denetimi
   * çerçeveye bakmadan uygulamak, aracın kendi Laravel desteğini reddetmesi
   * demek olurdu.
   */
  const clash = docroots.find((d) => rel(d) === rel(name));
  if (clash && !force && framework !== 'laravel') {
    throw new UserError(
